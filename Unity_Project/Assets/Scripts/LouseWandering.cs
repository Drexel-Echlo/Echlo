...
public class LouseWandering : MonoBehaviour
{
    ...
    public void Update()
    {
        wanderSpeed = Random.Range(4.0f, 6.0f);

        for ( int i = 0; i < 4; i++)
        {
            if (GameObject.Find("Food" + (i + 1).ToString()) == null)
            {
                patrolPoint[i] = GameObject.Find("MainPlayer").GetComponent<Transform>(); 
            }
            else
            {
                patrolPoint[i] = GameObject.Find("Food" + (i + 1).ToString()).GetComponent<Transform>();
            }
        }
